    ...
    <div className={className}>
      ...
      <div ref={wrapRef} className="relative">
        <input
          ...
          className="w-full px-4 py-3 pr-10 rounded-xl border-2 border-gray-100 dark:border-gray-800 focus:border-brand-400 focus:outline-none focus:ring-4 focus:ring-brand-50 bg-gray-50/50 focus:bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100 placeholder-gray-400 transition-colors duration-150"
        />
        {loading && (
        ...
        {open && results.length > 0 && (
          <div className="absolute z-20 mt-1 w-full bg-white dark:bg-gray-900 rounded-xl shadow-elevated border border-gray-100 dark:border-gray-800 max-h-64 overflow-y-auto">
            {results.map((r, i) => (
              <button
                key={i}
                type="button"
                onClick={() => handlePick(r)}
                className="w-full text-left px-4 py-2.5 text-sm text-gray-700 dark:text-gray-300 hover:bg-brand-50 hover:text-brand-700 border-b border-gray-50 last:border-0 transition-colors"
              >
                <div className="flex items-start gap-2">
                  <svg className="w-4 h-4 text-gray-400 dark:text-gray-500 mt-0.5 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17.657 16.657L13.414 20.9a2 2 0 01-2.828 0l-4.243-4.243a8 8 0 1111.314 0z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 11a3 3 0 11-6 0 3 3 0 016 0z" /></svg>
        ...
        {open && !loading && results.length === 0 && query.trim().length >= 3 && (
          <div className="absolute z-20 mt-1 w-full bg-white dark:bg-gray-900 rounded-xl shadow-elevated border border-gray-100 dark:border-gray-800 px-4 py-3 text-sm text-gray-500 dark:text-gray-500">
