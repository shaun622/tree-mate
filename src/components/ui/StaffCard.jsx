...
export default function StaffCard({ staff, onClick }) {
  ...
  return (
    <div className="flex items-center gap-3 p-3 rounded-xl hover:bg-gray-50 cursor-pointer transition-colors" onClick={onClick}>
      {staff.photo_url ? (
        <img src={staff.photo_url} alt={staff.name} className="w-10 h-10 rounded-full object-cover" />
      ) : (
        <div className="w-10 h-10 rounded-full bg-tree-100 flex items-center justify-center text-tree-600 font-semibold text-sm">
