      ...
      <div className="px-4 py-4 space-y-4">
        {surveys.length > 0 && (
          <Card className="p-4 text-center">
            ...
            <p className="text-sm text-gray-500 dark:text-gray-500">{surveys.length} responses</p>
          </Card>
        )}
          ...
          <div className="space-y-2">
            {surveys.map(s => (
              <Card key={s.id} className="p-4">
                ...
                {s.comment && <p className="text-sm text-gray-700 dark:text-gray-300">{s.comment}</p>}
                <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">{formatDate(s.created_at)}</p>
              </Card>
            ))}
          </div>
        )}
