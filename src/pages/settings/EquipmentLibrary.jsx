  ...
  return (
    <PageWrapper>
      <Header title="Equipment Library" back="/settings" rightAction={
        <button onClick={() => { setEditing(null); setForm({ name: '', category: 'cutting', hourly_rate: '', notes: '' }); setShowModal(true) }} className="p-2 hover:bg-gray-100 rounded-full">
      ...
      <div className="px-4 py-4 space-y-4">
        ...
        <div className="flex items-center justify-between">
          <h3 className="text-sm font-semibold text-gray-900">Suggested Equipment</h3>
              ...
              <Card key={s.name} className="p-3 flex items-center justify-between">
                <div>
                  ...
                  <div className="flex items-center gap-2 mt-0.5">
                    ...
                    <span className="text-xs text-gray-500">${s.hourly_rate}/hr</span>
                  </div>
                </div>
        ...
        <div className="flex gap-2 overflow-x-auto pb-1 no-scrollbar">
          <button onClick={() => setFilterCat('')} className={`px-3 py-1.5 rounded-lg text-xs font-medium whitespace-nowrap ${!filterCat ? 'bg-tree-500 text-white' : 'bg-white border border-gray-200 text-gray-600'}`}>All</button>
          {EQUIPMENT_CATEGORIES.filter(c => c.value).map(c => (
            <button key={c.value} onClick={() => setFilterCat(c.value)} className={`px-3 py-1.5 rounded-lg text-xs font-medium whitespace-nowrap ${filterCat === c.value ? 'bg-tree-500 text-white' : 'bg-white border border-gray-200 text-gray-600'}`}>{c.label}</button>
          ))}
        </div>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      <span className={`px-2 py-0.5 rounded text-xs font-medium ${CATEGORY_COLORS[eq.category] || 'bg-gray-100 text-gray-700'}`}>{eq.category}</span>
                      <span className="text-sm text-gray-500">${eq.hourly_rate}/hr</span>
                    </div>
