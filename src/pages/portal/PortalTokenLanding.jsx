  ...
  if (error) return (
    <div className="min-h-screen bg-gradient-page flex items-center justify-center p-6">
      <Card className="p-6 text-center max-w-sm w-full">
        ...
        <Link to="/portal/login" className="text-tree-600 text-sm font-medium">Go to Portal Login</Link>
      </Card>
    </div>
  )
    ...
    <div className="min-h-screen bg-gradient-page flex items-center justify-center p-6">
      <div className="w-full max-w-sm space-y-4">
        <div className="text-center">
          ...
          <h1 className="text-xl font-bold text-gray-900">Your Job Portal</h1>
        </div>

        <Card className="p-4">
          <p className="text-sm text-gray-500">Site</p>
          <p className="font-medium text-gray-900">{site?.address}</p>
          {site?.clients?.name && <p className="text-sm text-gray-500 mt-1">{site.clients.name}</p>}
        </Card>

        <Button onClick={() => navigate(`/portal/setup/${token}`)} className="w-full">Set Up Portal Account</Button>
        <p className="text-center text-sm text-gray-500">
          Already have an account? <Link to="/portal/login" className="text-tree-600 font-medium">Sign In</Link>
        </p>
      </div>
    </div>
  )
