...
export default function Jobs() {
  ...
  const JobCard = ({ job, compact = false, done = false, invoiceView = false, quotedView = false, scheduledView = false }) => {
    ...
    return (
      <div className={`w-full text-left bg-white rounded-2xl shadow-card overflow-hidden border border-gray-100/80 hover:border-tree-200 hover:shadow-card-hover hover:-translate-y-0.5 transition-colors duration-150 ${done ? 'opacity-60' : ''}`}>
        <button
          type="button"
          onClick={handleClick}
            ...
            {job.status === 'approved' ? (
              <>
                <button
                  ...
                  className="flex-1 flex items-center justify-center gap-1.5 py-2.5 text-[11px] font-semibold text-gray-500 hover:text-gray-700 hover:bg-gray-50 transition-colors duration-150"
                >
                ...
                <button
                  ...
                  className="flex-1 flex items-center justify-center gap-1.5 py-2.5 text-[11px] font-semibold text-tree-600 hover:text-white hover:bg-tree-600 transition-colors duration-150"
                >
    ...
    <PageWrapper width="wide">
      ...
      <Header title="Jobs" subtitle="Track every job from enquiry to completion" rightAction={
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => openQuoteModal(null, null)} className="px-3 py-1.5 text-xs font-semibold text-tree-600 border-2 border-tree-200 rounded-xl hover:bg-tree-50 transition-colors duration-150 active:scale-95">
            Quick Quote
          </button>
          <button onClick={() => setShowModal(true)} className="px-3 py-1.5 text-xs font-semibold text-white bg-tree-600 rounded-xl hover:bg-tree-700 shadow-button transition-colors duration-150 active:scale-95 flex items-center gap-1">
