    ...
    <PageWrapper>
      <div className="md:hidden">
        <Header title="Invoices" back="/settings" rightAction={
          <button onClick={() => navigate('/invoices/new')} className="p-2 hover:bg-gray-100 dark:hover:bg-gray-800 dark:bg-gray-800 rounded-full">
            ...
            {invoices.map(inv => (
              <Card key={inv.id} hover onClick={() => navigate(`/invoices/${inv.id}`)} className="p-4">
                <div className="flex items-center justify-between mb-1">
                  <p className="font-medium text-gray-900 dark:text-gray-100">{inv.invoice_number || 'Draft'}</p>
                  <Badge variant={badgeVariant(inv.status)}>{statusLabel(inv.status)}</Badge>
                </div>
                <div className="flex items-center justify-between">
                  <p className="text-sm text-gray-500 dark:text-gray-500">{clientMap[inv.client_id]?.name || 'Unknown'}</p>
                  <p className="font-semibold">{formatCurrency(inv.total || 0)}</p>
                </div>
                <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">{formatDate(inv.created_at)}</p>
              </Card>
            ))}
          </div>
        )}
